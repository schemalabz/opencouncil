 ...
 *   npx tsx scripts/backfill-subject-images.ts --window 10000 --limit 500
 *   npx tsx scripts/backfill-subject-images.ts --force --limit 10   # redraw even if an image exists
 *
 * The candidates are the newest `--window` subjects the landing can show
 * (released meetings, public cities, discussed subjects), minus those that
 * have an image already — one listing of the bucket folder says which. What
 * is left goes in the landing page's order of importance, so `--limit 50`
 * draws the 50 undrawn subjects a visitor is most likely to see, and a second
 * run draws the next 50. The pipeline and the read route cover the rest.
 */
...
import { generateImageForSubject, isSubjectImageGenerationEnabled, listSubjectsWithImages, type GenerateOutcome } from '../src/lib/subjectImages';
...
async function main() {
    const argv = await yargs(hideBin(process.argv))
        ...
        .option('window', { type: 'number', default: 2000, describe: 'How many of the newest subjects to consider' })
    ...
    const candidates = await getSubjectsForImageBackfill({ cityId: argv.city, monthsBack: argv.months, take: argv.window });
    const drawn = argv.force ? new Set<string>() : await listSubjectsWithImages();
    const undrawn = candidates.filter((row) => !drawn.has(row.id));
    const subjects = rankSubjectIdsForImages(undrawn).slice(0, argv.limit);
