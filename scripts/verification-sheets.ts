...
import { deriveMeetingFacts } from '@/lib/derivation/deriveMeetingFacts';
...
async function main() {
    ...
    const bodies = await prisma.administrativeBody.findMany({
        where: { ...(CITY ? { cityId: CITY } : {}), decisionConventions: { not: undefined } },
    ...
    for (const b of bodies) {
        ...
        const conv = asObj(b.decisionConventions) ?? {};
        const checks: Check[] = [];
        const lines: string[] = [`# ${b.cityId} / ${b.name}`, '', `Conventions: present list **${conv.presentListMeaning}**, per-decision list as attendance **${conv.statesPerDecisionAttendance}**, per-vote absence **${conv.statesPerVoteAbsence}**, secretary left out of the list **${conv.listOmitsSecretary ?? false}**, confirmed by a person **${(asObj(conv.provenance)?.source) === 'manual'}**.`, ''];
        ...
        for (const m of meetings) {
            ...
            const decisions = await prisma.decision.findMany({ where: { subject: { cityId: m.cityId, councilMeetingId: m.id }, extractorVersion: '4' }, select: { ada: true, subjectId: true, extraction: true, subject: { select: { agendaItemIndex: true } } } });
            ...
            for (const d of decisions) {
                const r = asObj(d.extraction) as Reading | null; if (!r || !d.ada) continue;
                const rc = asObj(r.rollCall); const presentIds = strs(rc?.presentIds); const absentIds = strs(rc?.absentIds);
                const list = strs(asObj(r.decisionAttendance)?.presentIds);
                ...
                if (list.length) for (const pid of presentIds) {
                    const o = office(pid, b.id, m.dateTime) ?? (asObj(r.presidedBy)?.personId === pid ? 'προεδρεύων' : null);
                    if (!o) continue;
                ...
                if (list.length && conv.statesPerDecisionAttendance === true) {
                    const exempt = presentIds.filter(pid => { const o = office(pid, b.id, m.dateTime); return o === 'Πρόεδρος' || (conv.listOmitsSecretary === true && o === 'Γραμματέας') || asObj(r.presidedBy)?.personId === pid; });
                    const unexplained = presentIds.filter(pid => !list.includes(pid) && !exempt.includes(pid) && !outForVote.some(c => c.personId === pid));
        ...
        if (asObj(conv.provenance)?.source !== 'manual') checks.push({ rank: CONFIRM, text: `The conventions are profiled, not confirmed. Once the checks above hold, confirm them in the admin form (city → Όργανα → ${b.name} → Επιβεβαίωση κανόνων ανάγνωσης).` });
