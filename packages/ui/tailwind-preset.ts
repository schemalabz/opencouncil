...
/**
 * Shared Tailwind preset for every OpenCouncil app (main app, services/notis).
 * Consumers add their own `content` globs (including packages/ui/src) and any
 * app-specific `safelist`; everything theme-shaped lives here so the apps
 * cannot drift apart. CSS variables referenced here are defined in
 * ./src/styles/tokens.css, which consumers @import in their globals.
 */
