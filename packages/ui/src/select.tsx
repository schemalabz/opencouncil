...
type PointerDownOutsideEvent = Parameters<
  NonNullable<React.ComponentPropsWithoutRef<typeof SelectPrimitive.Content>["onPointerDownOutside"]>
>[0]
...
>(({ className, children, position = "popper", onCloseAutoFocus, onEscapeKeyDown, onPointerDownOutside, onFocus, ...props }, ref) => {
  ...
  const handlePointerDownOutside = React.useCallback((event: PointerDownOutsideEvent) => {
    // Allow normal pointer down outside behavior
...
>(({ className, children, onFocus, ...props }, ref) => {
  ...
  const handleFocus = React.useCallback((event: React.FocusEvent<HTMLDivElement>) => {
    // Stop propagation to prevent focus events from reaching parent focus scopes
    // This prevents the "too much recursion" error when Select is inside Sheet/Dialog
    event.stopPropagation();
    onFocus?.(event);
  }, [onFocus]);
  ...
  return (
    <SelectPrimitive.Item
      ref={ref}
      className={cn(
        "relative flex w-full cursor-default select-none items-center rounded-[6px] py-1.5 pl-8 pr-2 text-sm outline-none focus:bg-foreground/[0.06] focus:text-foreground data-[highlighted]:bg-accent data-[highlighted]:text-accent-foreground hover:bg-accent hover:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
        className
      )}
      onFocus={handleFocus}
      {...props}
