...
// Geocode a free-text address to coordinates (Google Geocoding, biased to Greece) so the
// landing search can fly the map to a typed address. The key stays server-side.
...
export async function GET(req: NextRequest) {
    ...
    const url =
        ...
        `&region=gr&language=el&key=${env.GOOGLE_API_KEY}`;
