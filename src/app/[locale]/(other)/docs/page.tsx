...
export default async function ApiDoc({ params }: Props) {
  const [{ locale }, spec, user] = await Promise.all([params, getSpec(), getCurrentUser()]);
    ...
    <div className="mb-6 rounded-md border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-700">
      ...
      {hiddenPaths > 0 && (
        <>
          ...
          to see endpoints available to authenticated users. Admin and superadmin
          endpoints require elevated access.
        </>
      )}
    </div>
  );
