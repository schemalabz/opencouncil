...
import { DEMO_SCENARIO_BY_REALM } from './config'
...
export default function SearchDemo({ realm }: { realm: Realm }) {
    ...
    const tScenario = useTranslations(`about.demos.search.byRealm.${DEMO_SCENARIO_BY_REALM[realm]}`)
        ...
        <BrowserFrame url={`${getRealmDomain(realm)}/search`} className="w-full">
            <div className="bg-white p-3 md:p-4">
                ...
                <div className="flex items-center gap-2 rounded-lg border border-border bg-gray-50/80 px-3 py-2 mb-4">
                    ...
                    <span className="text-sm text-foreground">{tScenario('query')}</span>
                ...
                <div className="space-y-3">
                    {RESULT_DATES.map((date, i) => (
                        <div key={i} className="rounded-lg border border-border/50 p-3 hover:border-border transition-colors">
                            {/* Subject title */}
                            <p className="text-[13px] font-medium leading-snug">{tScenario(`results.${i}.subject`)}</p>

                            {/* Meta row */}
                            <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-1.5 text-[10px] text-muted-foreground">
                                <span>{tScenario(`results.${i}.city`)}</span>
                                ...
                                <span className="flex items-center gap-0.5">
                                    ...
                                    {tScenario(`results.${i}.speaker`)}
                                </span>
                                <span className="flex items-center gap-0.5">
                                    <MapPin className="h-2.5 w-2.5" />
                                    {tScenario(`results.${i}.location`)}
                                </span>
                            </div>
                            ...
                            <p
                                ...
                                dangerouslySetInnerHTML={{ __html: tScenario.raw(`results.${i}.highlight`) as string }}
                            />
                        </div>
                    ))}
                </div>
