 ...
 * and on keyboard focus. While hidden it takes no pointer events, or a tap on
 * the image would land on an invisible Regenerate.
 */
export function SubjectImageAdminControls({ subjectId, onChanged, className }: { subjectId: string; onChanged: () => void; /** Overrides where the bar sits in the parent, e.g. along its top when the foot holds text. */ className?: string }) {
    ...
    return (
        <>
            <div className={cn("pointer-events-none absolute inset-x-0 bottom-0 flex justify-end gap-2 bg-gradient-to-t from-black/60 to-transparent p-3 opacity-0 transition-opacity focus-within:pointer-events-auto focus-within:opacity-100 group-hover:pointer-events-auto group-hover:opacity-100", className)}>
