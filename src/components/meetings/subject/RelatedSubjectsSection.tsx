 ...
 *
 * A row also needs the subject's speaking statistics, for its minutes and
 * party dots. The search page's list container fetches those on the client,
...
export async function RelatedSubjectsSection({ seed, cityName }: { seed: RelatedSubjectSeed; cityName: string }) {
    ...
    const level = (scope: RelatedScope, subjects: SearchResultLight[]): RelatedLevel => {
        return { scope, subjects: subjects.map(subject => ({ ...subject, statistics: statistics.get(subject.id) })) };
    };
