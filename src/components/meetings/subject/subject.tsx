    ...
    return (
        <div className="min-h-screen bg-background">
            ...
            {/* The subject's own title, and the page's h1 — it used to live only in the
                ...
                gradient, as they do on the landing card. The illustration runs edge to
                edge as a banner: a fixed height, cropped from the middle, so a wide
                screen gets a wide picture and not a tall one. Its text keeps to the
                reading column below, so the title starts where the summary does.
                `group` + `relative` is what the admin overlay hangs off. */}
            <header className="group relative h-56 w-full overflow-hidden bg-muted md:h-72 lg:h-80 xl:h-96">
                <SubjectImage subjectId={subject.id} alt="" topic={topic} version={imageVersion} loading="eager" className="absolute inset-0" />
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 via-black/50 to-transparent pt-20 text-white">
                    <div className="relative mx-auto max-w-6xl px-3 pb-5 pr-12 md:px-6 md:pb-6 md:pr-16">
                        {topic && (
                ...
                {isSuperAdmin && (
                    <SubjectImageAdminControls subjectId={subject.id} onChanged={() => setImageVersion(Date.now())}>
                        {/* The metadata dialog's trigger rides in the same strip, as one more tool. */}
                        <SubjectAdminControls
                            subject={subject}
                            cityId={meeting.cityId}
                            meetingId={meeting.id}
                            className={cn(adminToolClass, 'w-8 px-0')}
                        />
            ...
            {/* The page in two columns: the content — summary, context, the
                ...
                discussion (the actual meat) under closed boxes. A flex gap, not
                space-y: the phone-only actions are display:none on a wide screen,
                and space-y would still count them. */}
            <div className="mx-auto flex max-w-6xl flex-col gap-6 px-3 py-4 md:px-6 md:py-6">
