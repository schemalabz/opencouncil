...
/**
 * The refraction map for the Chromium-only rim effect: a horizontal ramp
 * that is neutral (128) across the middle and displaces the backdrop at
 * the two edges, so the page behind bends at the rim the way it would
 * through a curved glass. Red drives x, green (constant 128) leaves y alone.
 */
const REFRACTION_MAP = "data:image/svg+xml;utf8," + encodeURIComponent(
    "<svg xmlns='http://www.w3.org/2000/svg' width='200' height='10'><defs><linearGradient id='g' x1='0' x2='1'>"
    + "<stop offset='0' stop-color='rgb(0,128,0)'/><stop offset='0.07' stop-color='rgb(128,128,0)'/>"
    + "<stop offset='0.93' stop-color='rgb(128,128,0)'/><stop offset='1' stop-color='rgb(255,128,0)'/>"
    + "</linearGradient></defs><rect width='200' height='10' fill='url(#g)'/></svg>",
);
const REFRACTION_FILTER_ID = 'oc-lens-refract';
...
export const TimelineLens = forwardRef<LensHandle, TimelineLensProps>(function TimelineLens(
    ...
    // Chromium alone renders an SVG filter inside backdrop-filter; everywhere
    // else the plain blur stands. Decided after mount so the server and the
    // first client render agree.
    const [refract, setRefract] = useState(false);
    useEffect(() => {
        setRefract(!compact && 'userAgentData' in navigator && typeof CSS !== 'undefined' && CSS.supports('backdrop-filter', `url(#${REFRACTION_FILTER_ID})`));
    }, [compact]);
        ...
        >
            {refract && (
                <svg aria-hidden className="absolute h-0 w-0">
                    <filter id={REFRACTION_FILTER_ID} x="0" y="0" width="100%" height="100%" colorInterpolationFilters="sRGB">
                        <feImage href={REFRACTION_MAP} preserveAspectRatio="none" result="map" />
            ...
            <div
                ...
                style={{ height, backdropFilter: refract ? `url(#${REFRACTION_FILTER_ID}) blur(16px) saturate(1.4)` : undefined }}
            >
