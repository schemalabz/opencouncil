        ...
        new Paragraph({
            ...
            children: [
                ...
                new ExternalHyperlink({
                    children: [new TextRun({
                        // getRealmDomain, not realmBaseUrl: this renderer is
                        // reachable from a client module, and a document handed
                        // to a municipality should name production, not a preview.
...
const createTranscriptSection = ({ transcript, people, meeting }: Pick<MeetingDataForExport, 'transcript' | 'people' | 'meeting'>) => {
    ...
    transcript.forEach((speakerSegment) => {
        ...
        const partyLabel = party
            ? isPartyHead ? `(${party.name_short}, Επικεφαλής) ` : `(${party.name_short}) `
            : '';
        const children = [
            new TextRun({
        ...
        if (role) {
            children.push(new TextRun({
                text: `${simplifyRoleName(role.name)} `,
                size: 20, // 10pt
                color: '666666'
            }));
        }
