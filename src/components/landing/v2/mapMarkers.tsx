...
/** The translate function shape from useTranslations('landingV2') — passed into the plain-DOM
 *  marker factories, which run outside React and so can't call the hook themselves. */
...
/**
 * Donut cluster marker — a topic-segmented ring with the total count on a white
 * centre disc and each topic's (white) icon riding on its segment. Style ported
 * from the CivicMap redesign (PR #472); geometry/SVG live in ./donut.ts.
 */
...
/* Desktop floating tooltip above the selected subject's map pin: category + title,
   then address + date, then a "view subject" affordance. Rendered into a Mapbox popup
...
export function DesktopSubjectTooltip({
    ...
    return (
        <div className="relative w-[300px] max-w-[80vw] font-relative">
            {/* The shared SubjectCard (preview) — navigation goes through onView since the
                Mapbox popup has no router context for a real Link. */}
            <SubjectCard subject={subject} variant="preview" onView={onView} onClose={onClose} className="shadow-xl" />
            {/* little tail pointing down toward the pin */}
...
/* white popover listing the subjects at one map point (opened from a "+N" marker).
   Positioned at the marker's screen point inside the relative map container. */
/* Floating list opened by a city-hall marker — a municipality's subjects that have no
   specific location. Items link straight to their subject pages (they can't be placed on
   the map, so there's no map-selection). Positioned at the marker's screen point. */
...
export function createSubjectPin(
    ...
    if (group.length > 1) {
        // A co-located pin stands in for several subjects — size its circle like the
        // municipality (city-hall) markers (h-9) so the "+N" badge sits at the same corner
        // offset as their count badge instead of overlapping a smaller (h-7) circle.
...
export function createDonutMarker(map: MapboxMap, members: LandingSubject[], t: TFn): SubjectPin {
    ...
    el.addEventListener('click', (e) => {
        ...
        // a couple of clicks break the donut into individual pins
 ...
 * cells (per integer zoom); a cell over CLUSTER_THRESHOLD collapses into a single donut,
 * everything else renders as an individual pin / "+N" marker. The selected subject's group
 * is pulled out of clustering and always drawn as its own highlighted pin (so a selection
 * stays visible even when zoomed out into an area that would otherwise collapse it).
 */
