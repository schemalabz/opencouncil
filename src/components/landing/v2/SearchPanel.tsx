...
/* The search field itself (icon · input · clear), shared by the desktop dropdown and the
   ...
   municipality filter (clearing the text) or geocodes an address; the caller's onAfterSubmit
   then closes/blurs. `className` carries the per-context shadow. */
        ...
        >
            ...
            <input
                ...
                onKeyDown={(e) => {
                    ...
                    // a category/municipality query applies that filter (clearing the text);
...
export function DesktopSearch({
    ...
    // Close on outside click or Escape while open. The native date picker renders
    // its calendar outside the dropdown, so ignore pointer events on date inputs.
    useEffect(() => {
        if (!open) return;
...
export function MobileSearchOverlay({
    ...
    // Opened via the filters icon → don't steal focus (no keyboard); instead bring the first
    // active filter into view so the user lands on what's already selected.
    useEffect(() => {
