    ...
    return (
        ...
        >
            ...
            <div className="relative aspect-[7/4] w-full overflow-hidden bg-muted">
                ...
                <span
                    className={cn(
                        ...
                        onClose ? 'max-w-[calc(100%-6.25rem)]' : 'max-w-[calc(100%-4.5rem)]',
                    )}
                ...
                {/* The map preview is the one surface that spans municipalities, so the
                    logo stays there too: left of the × rather than under it. */}
                {subject.cityLogo && (
                    <span className={cn('absolute top-3 flex h-9 w-9 items-center justify-center overflow-hidden rounded-full bg-white p-1 shadow-sm', onClose ? 'right-10' : 'right-3')}>
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 via-black/45 to-transparent px-4 pb-3 pt-12 text-white">
                    ...
                    <h3
                        className={cn(
                            'min-w-0 pr-9 text-lg font-bold leading-tight text-white',
                ...
                {/* One robot for the picture and the summary alike; opening it says so. */}
                <AIDisclosure className="absolute bottom-3 right-3 z-10" />
            </div>
            ...
            <div className={cn('flex min-w-0 flex-col gap-2', preview ? 'px-3 py-2' : 'px-4 pb-3 pt-3')}>
                ...
                <div className={cn('flex items-center', preview ? 'mt-0.5' : 'mt-1')}>
                    <SubjectPageLink
                        surface={surface}
                        href={subject.href}
                        onView={onView}
