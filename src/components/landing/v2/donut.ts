/**
 * Donut-cluster geometry + SVG, ported from the CivicMap redesign
 * (PR #472, src/lib/map/donut.ts) and adapted to the landing's clusters.
 *
 * A donut is a topic-segmented ring with the cluster's total count on a white
 * centre disc; each named segment carries its topic's (white) icon on the ring,
 * so a glance shows which subject areas the cluster holds, not just their count.
 * The CivicMap version reads mapbox cluster accumulator properties; here we
 * derive the same segments straight from the grouped LandingSubject members.
 */
...
/**
 * Topic distribution of a cluster's members, ordered by count: at most
 * DONUT_MAX_SEGMENTS segments; smaller topics and untopiced subjects merge into
 * a final neutral "other" segment.
 */
...
/**
 * Positions a topic icon on every named segment whose arc is long enough to hold
 * a legible one — at the segment's mid-angle and the band's mid-radius. Slivers
 * too thin (and the neutral "other" bucket) get none.
 */
 ...
 * total count on a white centre disc. The caller overlays per-segment topic
 * icons (donutSegmentIcons) on the ring band around it.
 */
