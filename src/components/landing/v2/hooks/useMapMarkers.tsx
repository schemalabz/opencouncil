...
import { CENTER_QUERY_MOVE_RATIO, clusterCellDegrees, stylePin, type LandingView, type SubjectPin } from '../landingCore';
import {
    groupByLocation,
...
/**
 * Capture the map view on every pan/zoom (moveend also fires at the end of a zoom) so the
 * list + clustering react to it. Re-clusters only when the cell size changes; resolves the
 * municipality under the center (for the "view its page" button); finishes a pending
 * co-located / city-hall pan by opening its box at the now-centered point; otherwise closes
 * the boxes and publishes the new viewport. Selection pans set `suppressViewCapture` so
 * they don't refilter the list.
 */
export function useMapViewCapture({
    ...
    // Center of the last /api/cities/at query — the gate below only re-queries once the map
    // center has moved a meaningful fraction of the viewport away from it.
    const lastCenterQueryRef = useRef<{ lng: number; lat: number } | null>(null);
    useEffect(() => {
        if (!mapInstance) return;
        const capture = () => {
            // re-cluster only when the cell size changes (React bails out if it's unchanged),
            // so small zooms within the same level don't rebuild/flip the donuts
            setClusterCell(clusterCellDegrees(mapInstance.getZoom()));
            setMapZoom(mapInstance.getZoom());

            const center = mapInstance.getCenter();
            const bounds = mapInstance.getBounds();
            // Which municipality is under the map center → drives the "view its page" button.
            // Skip the lookup unless the center moved more than CENTER_QUERY_MOVE_RATIO of the
            // viewport since the last query, so a pure zoom-in/out (center unchanged) or a tiny
            // pan makes no network call. The threshold scales with the viewport → same feel at
            // any zoom.
            if (bounds) {
            ...
            // a "+N" pan finished → open the box at the now-centered point (don't close it)
        ...
        // setters + refs are stable; effect only re-subscribes when the map instance changes
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [mapInstance]);
}

/**
 * Build the subject pins for the current viewport and keep them in sync. Subjects at the
 * exact same point merge into one "+N" marker (its co-located box opens on click); dense
 * grid cells collapse into donuts; the selected subject always stays its own highlighted
 * pin (see buildSubjectPins). A separate effect restyles pins in place on selection change.
 */
export function useSubjectMarkers({
    ...
    useEffect(() => {
        // Subject pins show wherever subject data is active (desktop subjects view; mobile home).
        if (!mapInstance || !subjectsActive) return;

        const openCoLocated = (group: LandingSubject[]) => {
            // opening a "+N" box deselects any selected subject so its preview/tooltip closes
            onClearSelectionRef.current();
            const lng = group[0].lng;
            const lat = group[0].lat;
            const c = mapInstance.getCenter();
            const willMove = Math.abs(c.lng - lng) > 1e-6 || Math.abs(c.lat - lat) > 1e-6;
            if (willMove) {
                // center the location (like a list selection); the box opens after the pan
                suppressViewCaptureRef.current = true;
                pendingCoLocatedRef.current = group;
                mapInstance.easeTo({ center: [lng, lat], duration: 400 });
            } else {
...
/**
 * City-hall markers: one per municipality that has non-located subjects in view. Clicking
 * one clears the other map previews and opens that δήμος's GeneralSubjectsBox (after a pan
 * that centers its centroid, offset to clear the floating list / top controls).
 */
export function useGeneralCityMarkers({
    ...
    useEffect(() => {
        // City-hall markers show wherever subject data is active (desktop subjects; mobile home).
        if (!mapInstance || !subjectsActive) return;
        ...
        for (const city of visibleGeneralCities) {
            const { marker, root } = createGeneralCityMarker(mapInstance, city, () => {
                ...
                if (willMove) {
                    // center the centroid (like a subject selection); the box opens after the pan.
                    // desktop: shift right so it lands in the centre of the visible map box (the
                    // left ~400px is under the floating list). mobile: drop it a bit below centre
                    // (still horizontally centered) so the marker and its box clear the top controls.
                    suppressViewCaptureRef.current = true;
 ...
 * one runs `onOpenCity` (its city page). Only mounted in the 'municipalities' view.
 */
