...
import { Map as MapIcon, Landmark, MoreHorizontal, LogIn, LogOut, User, Phone, Mail, ArrowRight } from 'lucide-react';
...
import type { LandingView } from './landingCore';
...
export function LandingAside({
    ...
    // Auth UI depends on the client session, which can differ between the server render and
    // the first client render (next-auth's unseeded SessionProvider) → React #418 hydration
    // mismatch. Gate it on a client-mounted flag so both render the same placeholder (null),
    // then reveal the real state after mount.
    ...
    return (
        ...
        <div className="flex w-[80px] shrink-0 flex-col items-center bg-card pb-3 pt-1">
            ...
            <nav className="flex flex-1 flex-col items-center justify-center gap-2">
                {/* "Αρχική" (home) intentionally omitted on desktop for iteration 1 — returns in iteration 2. */}
                <RailItem
