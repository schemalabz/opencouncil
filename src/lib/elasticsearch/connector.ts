...
export class ConnectorService {
  ...
  /**
   * Extracts the last update time of the advanced snippet query
   */
  extractQueryUpdateTime(config: ConnectorConfig): string | undefined {
    try {
      // Get the first (default) filtering configuration
      const filteringConfig = config.filtering?.[0];
      if (!filteringConfig) {
      ...
      // Try to get the update time from active configuration first, then draft
      const updateTime = filteringConfig.active?.advanced_snippet?.updated_at ||
                         filteringConfig.draft?.advanced_snippet?.updated_at;
      
      return updateTime || undefined;
    } catch (error) {
      console.error('Error extracting query update time from connector config:', error);
      return undefined;
    }
  }
  ...
  async getConnectorStatus(): Promise<ConnectorStatus> {
    try {
      ...
      const queryUpdatedAt = this.extractQueryUpdateTime(config);
