...
describe('joinTranscriptSegments', () => {
  it('keeps two speaker tags of one person apart when asked, and joins them otherwise', () => {
    ...
    expect(joinTranscriptSegments(segments as any)).toHaveLength(1);

    const apart = joinTranscriptSegments(segments as any, { sameSpeakerTagOnly: true });
  ...
  it('never joins segments without a person, with or without the option', () => {
    ...
    expect(joinTranscriptSegments(segments as any)).toHaveLength(2);
    expect(joinTranscriptSegments(segments as any, { sameSpeakerTagOnly: true })).toHaveLength(2);
  });
