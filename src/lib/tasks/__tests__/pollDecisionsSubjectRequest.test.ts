...
describe('requestPollDecisionForSubject', () => {
    ...
    it('names the failing fields in the error, so an alert does not need a query', async () => {
        mockSubjectFindUnique.mockResolvedValue({ ...OUT_OF_AGENDA_SUBJECT, withdrawn: true });

        await expect(requestPollDecisionForSubject('subject-1')).rejects.toThrow(
            'nonAgendaReason=outOfAgenda, withdrawn=true',
        );
    });
