...
/**
 * Response shape for endpoints that send a single message:
 *   POST /workspaces/{ws}/channels/{ch}/messages       (SMS)
 *   POST /workspaces/{ws}/conversations/{id}/messages  (WhatsApp into existing conversation)
 */
...
/**
 * Response shape for `POST /workspaces/{ws}/conversations`. Top-level `id`
 * is the conversation ID; the new message ID lives under `initialMessage.id`
 * (or one of the other fallback paths Bird has used).
 */
...
type WelcomeTemplateParams = { userName: string; cityName: string };
...
}): Promise<{ success: boolean; error?: string; messageId?: string; conversationId?: string }> {
 ...
 */
export async function createOrUpdateConversation(input: CreateOrUpdateConversationInput): Promise<{
    success: boolean;
    error?: string;
    conversationId?: string;
    messageId?: string;
