...
function isStale(
    ...
    if (period.source !== VoicePrintConsentSource.PERSON) return false;
    // A deleted account leaves userId null, and null is also "nobody claims
    // the person": the two must not read as the same account.
    return period.userId === null || period.userId !== claimantUserId;
}
