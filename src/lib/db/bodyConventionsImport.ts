 ...
 * admin. `options.replace` names the exception: a key in it (`"<cityId>/<body
 * name>"`) is written even when the stored record is confirmed, for a record
 * the maintainer re-reviewed. A record naming a body the database does not
 * hold is reported, not created — a seed holds a few cities, production holds
 * them all.
 *
...
export async function importBodyConventions(
    ...
    options: { replace?: ReadonlySet<string> } = {},
    ...
    for (const r of records) {
        ...
        if (isConfirmedByPerson(stored.decisionConventions) && !options.replace?.has(key)) { result.confirmedSkipped.push(key); continue; }
