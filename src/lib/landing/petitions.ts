...
export function petitionFill(intensity: number): { background: string; text: string } {
    ...
    return {
        ...
        // that across the whole ramp with one colour — the flip lives at pct 81, the lowest
        // step where white carries it, giving a measured minimum of 4.60:1 over all 76 integer
        // steps (the contrast test scans every one).
        text: pct >= 81 ? '#ffffff' : '#000000',
    };
}
