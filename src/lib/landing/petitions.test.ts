...
/** The ramp endpoints, read from PETITION_BLUE itself ("change it here or nowhere") — a hue change
 *  must re-run the measurement, not silently leave the test on the old colour. */
const parseHsl = (v: string): [number, number, number] => {
    const m = v.match(/^hsl\((\d+) (\d+)% (\d+)%\)$/);
    if (!m) throw new Error(`unexpected hsl syntax: ${v}`);
...
const rampBackground = (pct: number): [number, number, number] => {
    const deep = parseHsl(PETITION_BLUE.deep);
    const pale = parseHsl(PETITION_BLUE.pale);
...
describe('petitionFill', () => {
    it('meets WCAG AA (4.5:1 — the badges are 12px text) at every integer step of the ramp', () => {
        // Every pct the ramp can produce (25..100), not a sparse sample: a 0.1-intensity sweep
        // once measured 11 of the 76 steps and missed the three failing ones right below the
        // old text flip.
        const white: [number, number, number] = [255, 255, 255];
        const black: [number, number, number] = [0, 0, 0];
        for (let pct = 25; pct <= 100; pct++) {
            const { text } = petitionFill((pct - 25) / 75);
            const bg = rampBackground(pct);
            const fg = text === '#ffffff' ? white : black;
            const ratio = contrast(fg, bg);
            expect({ pct, text, ratio, passes: ratio >= 4.5 }).toMatchObject({ passes: true });
        }
    });
});
