...
Administrators get write tools on their personal MCP URL: \`create_meeting\`, \`update_meeting\` and \`start_task\` for the municipalities they administer, and for superadmins also \`create_city\` and \`populate_city\`. If you see none of them, the account has no administrator rights. These tools change live data: confirm each write with the user before you call, and never call one a second time after an unclear result — read the state first (\`list_meetings\`, \`get_city\`). For an administrator, \`get_meeting\` also lists the pipeline tasks of the meeting with their status, so \`start_task\` is never a guess: check what ran and what failed before you start a step, and poll \`get_meeting\` after.`;
