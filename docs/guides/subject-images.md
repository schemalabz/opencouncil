...
3. **The backfill script.** `scripts/backfill-subject-images.ts` draws the subjects a visitor is most likely to see. It takes the newest `--window` subjects the landing can show: released meetings, public cities, discussed subjects. It removes the ones that have an image; one listing of the bucket folder says which. It ranks the rest in the landing page's order of importance, through the shared ranker in `src/lib/ranking/subjects.ts`. `--limit 50` then draws the first 50 of them, and a second run draws the next 50. The window is bounded because the discussion-time query binds one parameter per subject. The script is optional. It doubles as a test of the prompt across every kind of subject.
...
Library
- `SYSTEM_PROMPT`, `buildPrompt`: [`packages/subject-images/src/prompt.ts`](../../packages/subject-images/src/prompt.ts) (the system instruction with the style and the rules, and the user message built from the city, the title and the description)
...
- The AI disclosure on the landing card and the subject page does not know about manual uploads. The object carries no source metadata yet.
