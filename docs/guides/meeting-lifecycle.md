...
Meetings progress through defined stages based on completed tasks:

1. **Scheduled** - Initial state when meeting is created
2. **Agenda Processed** - Agenda PDF has been processed and subjects extracted
3. **Transcribed** - Audio/video has been transcribed to text
4. **Transcript Fixed** - Automatic transcript correction has been applied
5. **Human Reviewed** - Manual review and correction completed by human
6. **Summarized** - AI summarization and topic extraction completed
7. **Ready** - All required tasks completed, meeting fully processed
...
*   **Database Functions**:
    ...
    *   `src/lib/db/tasks.ts` (getMeetingTaskStatus, MeetingTaskType definitions, core processing task tracking)
*   **Frontend Components**:
...
1. Meeting stage is derived from the highest completed core processing task
2. A meeting is considered "ready" only when all core processing tasks are completed
3. Human review is tracked separately and required for meeting completion
